...
from components.DqnModel import DqnModel
from components.Rewards import Rewards

os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

class MVDqnPlayer(MVBasePokerPlayer):
    """
    Documentation for callback arguments given here:
    https://github.com/ishikota/PyPokerEngine/blob/master/AI_CALLBACK_FORMAT.md
    """

    def __init__(self, player_name="DqnPlayer", split_network=False, with_per=False, reward_system=1, *args, **kwargs):
        self.player_name = player_name
        super(MVDqnPlayer, self).__init__(*args, **kwargs)

        # Network Parameters which will be used
        self.out_layer_size = 4
        self.agent = DqnModel(name=player_name, alpha=0.05, gamma=1, input_layer_size=self.input_layer_size,
        ...
        self.episode_printout = config['log_settings'].getboolean('episode_printout_dqn')
