...
class DqnModel(object):
    ...
    def __init__(self, name, alpha, gamma, input_layer_size, number_of_parameters, out_layer_size, memory_size=10000,
        ...
        self.tensorboard_visualization = config['tensorboard'].getboolean('enable_dqn')

        # Hyperparameters
        self.memory = MemoryBuffer(max_size=memory_size, number_of_parameters=number_of_parameters)
        self.gamma = gamma
    ...
    def replay(self):
        ...
        q_target[batch_index, actions] = \
            rewards + self.gamma * q_eval[batch_index, max_actions.astype(int)] * dones
    ...
    def choose_action(self, state, street):
        ...
        if np.random.rand() <= self.epsilon:
            ...
            q_values = self.dqn_eval.predict(
                [state, np.ones(self.out_layer_size).reshape(1, self.out_layer_size)])
